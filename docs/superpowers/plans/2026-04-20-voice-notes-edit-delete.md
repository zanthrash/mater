# Voice Notes Edit & Delete Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add inline transcript editing and per-note deletion (with confirmation) to `VoiceNotesCard` in the dashboard asset detail page.

**Architecture:** `VoiceNotesCard` gains an `assetId` prop and calls `useUpdateAsset` internally. Edit/delete both manipulate the `voice_notes` array locally and call the existing `PUT /api/assets/:id` endpoint. No backend changes.

**Tech Stack:** React 18, TypeScript, TanStack React Query v5, Tailwind CSS v4, Axios
...
| File | Change |
|------|--------|
| `dashboard/src/components/VoiceNotesCard.tsx` | Rewrite with edit/delete UI + mutation logic |
| `dashboard/src/routes/assets/$id.tsx` | Add `assetId={id}` prop to `<VoiceNotesCard>` |

---

## Task 1: Rewrite VoiceNotesCard with edit and delete

**Files:**
- Modify: `dashboard/src/components/VoiceNotesCard.tsx`
...
```bash
git add dashboard/src/components/VoiceNotesCard.tsx
git commit -m "feat: add inline edit and delete to VoiceNotesCard"
```

---

## Task 2: Wire assetId in asset detail page

**Files:**
- Modify: `dashboard/src/routes/assets/$id.tsx` (line 167)

- [ ] **Step 1: Add `assetId` prop to the VoiceNotesCard call**
...
git commit -m "feat: pass assetId to VoiceNotesCard"
```
...
- [ ] **Step 2: Open an asset with voice notes**

Navigate to an asset detail page that has at least one voice note.

- [ ] **Step 3: Verify edit flow**
  - Click the pencil icon on a note → textarea appears pre-filled with the transcript
  - Edit the text → click Save → transcript updates in the UI
  - Click pencil again → click Cancel → original text is restored, no mutation fired
  - Pencil icon is not visible while another note is in delete-confirm mode

- [ ] **Step 4: Verify delete flow**
  - Click the trash icon on a note → "Delete this note? Yes / No" replaces the card body
  - Click No → card returns to normal, note still present
  - Click trash again → click Yes → note disappears from the list
  - If the deleted note was playing, audio stops

- [ ] **Step 5: Verify loading states**
  - During save/delete, Save/Yes/No/Cancel buttons are visually dimmed (`opacity-40`)
  - No double-submission possible while `isPending`
